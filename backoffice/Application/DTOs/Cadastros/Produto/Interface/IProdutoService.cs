...
public interface IProdutoService 
{
    ...
    Task<ProdutoViewModel> GetByIdAppAsync(int id, string? idEmpresa);

    Task<ProdutoViewModel> GetByIdAsync(int id, string? idEmpresa);

    Task<ProdutoViewModel> GetByNameAsync(string name, string? idEmpresa);

    Task AddAsync(ProdutoViewModel obj, string? idEmpresa);
    Task<IEnumerable<string>> GetNomesByIdsAsync(List<int> ids, string? idEmpresa);
