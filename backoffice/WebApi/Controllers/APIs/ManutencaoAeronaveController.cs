...
public class ManutencaoAeronaveController : ControllerBase
{
    ...
    {
        var returnMsg = new StringBuilder().Append("Registro de manutenção não encontrado");
        try
        {
            var manutencaoAeronave = await _manutencaoAeronaveService.GetByIdAsync(id);
            if (manutencaoAeronave is not null)
                returnMsg.Clear();
            else
                _logService.LogWarning(returnMsg.ToString());

            return string.IsNullOrEmpty(returnMsg.ToString()) ? Ok(manutencaoAeronave) : NotFound(returnMsg.ToString());
        }
        catch (Exception ex)
        {
            var errorMsg = returnMsg.Clear().Append($"Erro ao recuperar registro de manutenção pelo ID: {ex.Message}").ToString();
